...
export default function WorkforceProjectsPage() {
  ...
  const columns: DataColumn<WorkforceWorkItem>[] = [
    { key: "item", header: "Work item", render: (item) => <div><Link href={`/workforce/projects/${item.id}`} className="font-medium text-ink hover:text-signal focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-focusRing">{item.title}</Link><p className="mt-0.5 text-xs text-inkFaint">{projectNameById(item.project_id)}</p></div> },
