    ...
    {!loading && <div className="grid gap-6 xl:grid-cols-[360px_1fr]">
      ...
      <section>
        ...
        {result && <div className="space-y-6">
            ...
            <div className="flex flex-wrap items-start justify-between gap-4"><div><p className="text-xs uppercase tracking-wide text-inkSubtle">Decision preview</p><p className="mt-2 text-3xl font-semibold text-ink">{result.decision}</p></div><div className="flex flex-wrap gap-2"><StatusPill value={result.execution_plan} />{result.risk_classification && <StatusPill value={`${result.risk_classification} · ${result.risk_score}/100`} />}</div></div>
            <p className="mt-5 text-sm leading-6 text-slate-600">{result.reason}</p>
            <dl className="mt-6 grid gap-4 border-t border-slate-100 pt-5 text-xs sm:grid-cols-2"><div><dt className="text-inkSubtle">Next control</dt><dd className="mt-1 font-medium text-ink">{result.approval_required ? "Human approval and revalidation" : "Submit through the gateway"}</dd></div><div><dt className="text-inkSubtle">Planned connector</dt><dd className="mt-1 font-medium text-ink">{result.connector_provider}</dd></div><div><dt className="text-inkSubtle">Policy reason</dt><dd className="mt-1 font-mono text-slate-600">{result.reason_code}</dd></div><div><dt className="text-inkSubtle">Payload digest</dt><dd className="mt-1 break-all font-mono text-slate-600">{result.payload_digest}</dd></div></dl>
          </div>
