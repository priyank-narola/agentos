...
export default function HomePage() {
  return (
    <PublicShell>
      <main>
        <section className="marketing-hero overflow-hidden bg-ink text-white">
          <div className="mx-auto grid max-w-7xl gap-12 px-5 pb-20 pt-16 lg:grid-cols-[1.12fr_.88fr] lg:px-8 lg:pb-28 lg:pt-24">
            <div className="hero-copy max-w-3xl">
              <p className="motion-reveal motion-delay-1 inline-flex rounded-full border border-emerald-300/20 bg-emerald-300/10 px-3 py-1 text-xs font-semibold tracking-wide text-emerald-200">GOVERNED AI OPERATIONS</p>
              <h1 className="motion-reveal motion-delay-2 mt-6 text-5xl font-semibold leading-[1.03] tracking-[-0.045em] sm:text-6xl lg:text-7xl">Let agents work.<br /><span className="text-emerald-300">Keep humans accountable.</span></h1>
              <p className="motion-reveal motion-delay-3 mt-7 max-w-xl text-lg leading-8 text-slate-300">AgentOS is the operating layer for teams that need AI agents to plan useful work while every consequential action stays governed, reviewable, and provable.</p>
              <div className="motion-reveal motion-delay-4 mt-9 flex flex-wrap gap-3">
                <Link href="/signup" className="public-cta rounded-lg bg-white px-5 py-3 text-sm font-semibold text-ink transition hover:bg-slate-100">Start in sandbox →</Link>
                <Link href="/product" className="public-secondary-cta rounded-lg border border-slate-600 px-5 py-3 text-sm font-semibold text-white transition hover:border-slate-400 hover:bg-white/5">Explore the platform</Link>
              </div>
              <p className="motion-reveal motion-delay-5 mt-4 text-xs text-slate-500">Sandbox-first. No live money movement in this environment.</p>
            </div>
            <div className="hero-command-card motion-reveal motion-delay-3 relative rounded-2xl border border-slate-700 bg-slate-900/70 p-5 shadow-2xl shadow-black/20 lg:mt-3">
        ...
        <section className="mx-auto max-w-7xl px-5 py-20 lg:px-8 lg:py-28">
          ...
          <div className="mt-12 grid gap-5 md:grid-cols-3">{features.map(([title, body], index) => <article key={title} className={`public-feature-card motion-reveal rounded-xl border border-hairline bg-surface p-6 shadow-card motion-delay-${index + 1}`}><span className="grid h-9 w-9 place-items-center rounded-lg bg-signalSoft text-sm font-bold text-signal">0{index + 1}</span><h3 className="mt-6 text-lg font-semibold text-ink">{title}</h3><p className="mt-3 text-sm leading-6 text-inkSubtle">{body}</p><Link href={index === 1 ? "/workforce" : index === 2 ? "/evidence" : "/product"} className="mt-5 inline-block text-sm font-semibold text-signal hover:text-signalHover">Explore →</Link></article>)}</div>
        </section>
