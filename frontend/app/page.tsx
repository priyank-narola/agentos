    ...
    <main className="min-h-screen bg-[#f4f6f5] lg:flex">
      <aside className="border-b border-slate-200 bg-ink px-6 py-6 text-slate-300 lg:min-h-screen lg:w-64 lg:border-b-0 lg:border-r">
        ...
        <nav aria-label="Primary navigation" className="space-y-2">
          {["Overview", ["Runtime gateway", "/gateway"], ["Action requests", "/action-requests"], ["Approvals", "/approvals"], ["Agents", "/agents"], ["Tools", "/tools"], ["Resources", "/resources"], ["Policies", "/policies"], ["Policy evaluation", "/policy-evaluation"]].map((item, index) => {
        ...
        <div className="mt-16 border-t border-white/10 pt-5 text-xs leading-5 text-slate-500">Control center<br />External execution disabled</div>
      </aside>
      <section className="min-w-0 flex-1 px-5 py-7 lg:px-10 lg:py-9">
        <header className="mb-7 flex flex-wrap items-end justify-between gap-5 border-b border-slate-200 pb-7">
          ...
          <div className="border border-red-200 bg-white px-4 py-3"><p className="text-[10px] font-bold uppercase tracking-[0.16em] text-red-600">External execution</p><p className="mt-1 text-lg font-semibold text-red-800">DISABLED</p></div>
        </header>
