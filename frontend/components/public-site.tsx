...
export function PublicHeader() {
  return (
    <header className="public-header sticky top-0 z-40 border-b border-white/10 bg-ink/95 backdrop-blur">
      <div className="mx-auto flex h-16 max-w-7xl items-center justify-between px-5 lg:px-8">
        <Link href="/" className="public-wordmark flex items-center gap-2.5 text-white" aria-label="AgentOS home">
          <span className="public-mark grid h-8 w-8 place-items-center rounded-lg bg-signal text-sm font-bold">A</span>
        ...
        <nav aria-label="Public navigation" className="hidden items-center gap-7 text-sm text-slate-300 md:flex">
          {nav.map((item) => <Link key={item.href} href={item.href} className="public-nav-link transition hover:text-white">{item.label}</Link>)}
        </nav>
        <div className="flex items-center gap-3 text-sm font-medium">
          <Link href="/login" className="hidden text-slate-200 transition hover:text-white sm:inline">Sign in</Link>
          <Link href="/signup" className="public-cta rounded-lg bg-white px-3.5 py-2 text-ink transition hover:bg-slate-100">Start in sandbox</Link>
        </div>
      </div>
    </header>
  );
...
export function PublicShell({ children }: { children: ReactNode }) {
  return <div className="public-site min-h-screen bg-white"><PublicHeader />{children}<PublicFooter /></div>;
}
