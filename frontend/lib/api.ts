...
export type ActionEvidenceBundle = { evidence_version: string; exported_at: string; action_request: ActionRequest; approval?: { id: string; status: string; requested_by: string; decision_reason?: string | null; decided_by?: string | null; decided_at?: string | null; expires_at?: string | null } | null; execution_receipt: ExecutionReceipt; audit_events: Array<{ id: string; event_type: string; sequence?: number | null; occurred_at: string; event_data: Record<string, unknown> }>; integrity: { algorithm: "SHA-256"; digest: string; excluded_fields: string[] }; };
...
export const api = {
  ...
  updateWorkforceProject: (id: string, payload: Partial<Pick<WorkforceProject, "name" | "description" | "status" | "goal_id" | "owner_agent_id">>) => request<WorkforceProject>(`/api/v1/workforce/projects/${id}`, { method: "PATCH", body: JSON.stringify(payload) }),
  ...
  approve: (id: string, approver_principal_id: string, decision_reason: string) => request<Approval>(`/api/v1/approvals/${id}/approve`, { method: "POST", body: JSON.stringify({ approver_principal_id, decision_reason }) }),
  reject: (id: string, approver_principal_id: string, decision_reason: string) => request<Approval>(`/api/v1/approvals/${id}/reject`, { method: "POST", body: JSON.stringify({ approver_principal_id, decision_reason }) }),
